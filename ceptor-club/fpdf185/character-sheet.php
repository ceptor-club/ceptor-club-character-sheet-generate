<?php
    ...
    // Check if the avatarUrl is not empty
    $avatarUrl = $_POST['avatarUrl'];
    if (!empty($avatarUrl)) {
        // Decode the base64 image
        $avatarData = explode(',', $avatarUrl);
        $avatarImage = base64_decode($avatarData[1]);

        // Save the decoded image to a temporary file
        $tempFileName = 'temp_avatar.png';
        file_put_contents($tempFileName, $avatarImage);

        // Add the avatar image to the PDF
        $pdf->Image($tempFileName, 210, 250, 230);
